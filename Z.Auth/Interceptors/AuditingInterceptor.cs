...
namespace Z.Auth.Interceptors;

public class AuditingInterceptor(IAuthUserContext context) : SaveChangesInterceptor
{
    private readonly IAuthUserContext _context = context ?? throw new NullReferenceException("AuthUserContext must not be null.");
    ...
    public void UpdateEntities(DbContext? context)
    {
        ...
        foreach (var entry in context.ChangeTracker.Entries())
        {
            ...
            {
                if (entry.Entity is IMomentable loga)
                {
                    loga.CreatedAt = now;
                }
                if (entry.Entity is IAuditable audit)
                {
                    audit.Creator = _context?.User?.Username ?? "";
                }
            }
            ...
            if (entry.State == EntityState.Modified || HasChangedOwnedEntities(entry))
            {
                ...
                if (entry.Entity is IAuditable audit)
                {
                    audit.Modifier = _context?.User?.Username ?? "";
                }
            }
        }
    }
